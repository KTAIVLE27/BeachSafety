...
| [이대환](https://github.com/leeeedaehwan) | $\cdot$ Frontend <br/> $\cdot$ 메인 및 관리자 페이지 템플릿 구성 <br/> $\cdot$ 관제 페이지 지도 API 연결 <br/> $\cdot$ 실시간 이안류 위험도 예측 모델(데이터 전처리 및 시각화, 머신러닝 모델 설계)                                                                                |
